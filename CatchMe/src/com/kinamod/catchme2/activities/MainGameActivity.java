...
import com.kinamod.catchme2.containers.Bucket;
import com.kinamod.catchme2.containers.ExplosionContainer;
import com.kinamod.catchme2.containers.FallingObjectContainer;
import com.kinamod.catchme2.gameobject.BackgroundStars;
import com.kinamod.catchme2.gameobject.FuelBar;
import com.kinamod.catchme2.gameobject.HealthBar;
import com.kinamod.catchme2.util.CustomisedLogging;
import com.kinamod.catchme2.util.GameCanvas;
import com.kinamod.catchme2.util.MathsHelper;
import com.kinamod.catchme2.util.SoundPoolCatchMe;
...
public class MainGameActivity extends SwarmActivity {
	...
	private void checkCollisions() {
		...
		for (int bucketSide = 0; bucketSide < 4; bucketSide++) {
			...
			for (int whichCircle = 0; whichCircle < fOContainer.size(); whichCircle++) {
				...
				final boolean shouldICheck = fOContainer.shouldICheck(
						whichCircle, bucketHalfSize);
				if (!shouldICheck) {
	...
	private void makeCircleThread() {
		...
		circleThread = new Thread("circleThread") {
			...
			public void run() {
				...
				if (delay > 1050) {
					delay -= 10;
					logger.localDebugLog(2, "FallingFreq", "Next Fall: "
							+ delay);
				}
			...
			new Thread("musicThread") {
				...
				public void run() {
					...
					// }
						...
						logger.localDebugLog(2, "MediaPlayer",
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MAIN_GAME_ACTIVITY.getWindow().setFlags(
		...
		// setupGameScreen
		logger.localDebugLog(1, "TIMER", "    - - - - ALL Local Pointers: "
				+ (System.currentTimeMillis() - time)); // TODO
		...
		logger.localDebugLog(1, "TIMER",
				"Done Set Content View: " + (System.currentTimeMillis() - time));
		...
		logger.localDebugLog(1, "TIMER",
				"MGA Create Finished: " + (System.currentTimeMillis() - time));
	}
	...
	public void onStart() {
		...
		if (catchMe.isGameOver() || catchMe.isGameStopped()) {
			...
			startGame();
			logger.localDebugLog(1, "TIMER",
					"startGame-FULL: " + (System.currentTimeMillis() - time));
	...
	protected void onResume() {
		...
		if (restarting) {
			...
			catchMe.setPaused(false);
			logger.localDebugLog(1, "TIMER",
					"onResume: " + (System.currentTimeMillis() - time2));

		}

		logger.localDebugLog(1, "TIMER",
				"onResume: " + (System.currentTimeMillis() - catchMe.time2));
	}
	...
	private void setUpLocalPointers() {
		...
		logger.localDebugLog(1, "TIMER",
				"getMGAAct: " + (System.currentTimeMillis() - time)); // TODO
																		// DSJ
		time = System.currentTimeMillis(); // TODO DSJ

		fOContainer = new FallingObjectContainer(MAIN_GAME_ACTIVITY);
		logger.localDebugLog(1, "TIMER",
				"fOContainer: " + (System.currentTimeMillis() - time)); // TODO
																		// DSJ
		time = System.currentTimeMillis();
		bucket = new Bucket(MAIN_GAME_ACTIVITY);
		logger.localDebugLog(1, "TIMER",
				"bucket: " + (System.currentTimeMillis() - time)); // TODO
																	// DSJ
		time = System.currentTimeMillis();
		soundPool = new SoundPoolCatchMe(MAIN_GAME_ACTIVITY);
		logger.localDebugLog(1, "TIMER",
				"soundPool: " + (System.currentTimeMillis() - time)); // TODO
																		// DSJ
		time = System.currentTimeMillis();
		bgStars = new BackgroundStars(MAIN_GAME_ACTIVITY);
		logger.localDebugLog(1, "TIMER",
				"bgStars: " + (System.currentTimeMillis() - time)); // TODO
																	// DSJ
		time = System.currentTimeMillis();

		gameCanvasView = new GameCanvas(MAIN_GAME_ACTIVITY, MAIN_GAME_ACTIVITY);
		logger.localDebugLog(1, "TIMER",
				"gCanvasView: " + (System.currentTimeMillis() - time)); // TODO
																		// DSJ
	...
	private void startGame() {
		...
		resetGame();
		logger.localDebugLog(1, "TIMER",
				"resetGame: " + (System.currentTimeMillis() - time));
		time = System.currentTimeMillis(); // TODO DSJ

		logger.localDebugLog(1, "TIMER",
				"loadmusic: " + (System.currentTimeMillis() - time));
		time = System.currentTimeMillis(); // TODO DSJ
		makeThreads();
		logger.localDebugLog(1, "TIMER",
				"makeThreads: " + (System.currentTimeMillis() - time));
		time = System.currentTimeMillis(); // TODO DSJ
		makeBars();
		logger.localDebugLog(1, "TIMER",
				"makeBars: " + (System.currentTimeMillis() - time));
		time = System.currentTimeMillis(); // TODO DSJ
		gameExecutor = new ScheduledThreadPoolExecutor(1);
		gameExecutor.scheduleAtFixedRate(gameThread, 0, 1000 / 30,
				TimeUnit.MILLISECONDS);
		logger.localDebugLog(1, "TIMER",
				"makegameThread: " + (System.currentTimeMillis() - time));
		time = System.currentTimeMillis(); // TODO DSJ
		explosionKiller = new ScheduledThreadPoolExecutor(1);
		explosionKiller.scheduleAtFixedRate(explosionThread, 0, 500,
				TimeUnit.MILLISECONDS);
		logger.localDebugLog(1, "TIMER",
				"makeExplosionThread: " + (System.currentTimeMillis() - time));
		...
		logger.localDebugLog(1, "TIMER",
				"StartMusic: " + (System.currentTimeMillis() - time));
	...
	private void update() {
		while (!allTexturesLoaded()) {
			try {
				...
				logger.localDebugLog(2, "Wait To Start Game",
						" This won't get Interrupted...ironic");
			}
		}
	...
	private boolean allTexturesLoaded() {
		logger.localDebugLog(1, "BG || noBGGame,Buck,FO",
				(BackgroundStars.bitMapLoaded() || !BackgroundStars.isBGGame())
						+ " : " + Bucket.bitMapsLoaded() + " : "
						+ FallingObjectContainer.bitMapLoaded());
		return ((BackgroundStars.bitMapLoaded() || !BackgroundStars.isBGGame())
				&& Bucket.bitMapsLoaded() && FallingObjectContainer
					.bitMapLoaded());
	}
	...
	private void triggerGameAnimation(int delay) {
		...
		fRFill.scheduleAtFixedRate(fuelRefillThread, delay, 500,
				TimeUnit.MILLISECONDS);
	...
	private synchronized void upDateHighScore() {
		...
		// setting preferences
		prefs = MAIN_GAME_ACTIVITY.getSharedPreferences(CatchMe.PREF_FILE_NAME,
